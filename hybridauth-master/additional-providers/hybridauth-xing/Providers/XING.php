...
class Hybrid_Providers_XING extends Hybrid_Provider_Model_OAuth1 {
    ...
    */
    function getUserProfileById($userid) {
        $oResponse = $this->api->get('users/'.$userid);
    ...
    function searchUsers() {
        try {
            $oResponse = $this->api->get('users/find?keywords=fonpit,appmedia');
          //  $oTotal    = $oResponse->contacts->users;
          //  $iTotal    = $oResponse->contacts->total;
    ...
    function searchUsersByKeyword($keyword) {
      try {
        $oResponse = $this->api->get('users/find?keywords='.$keyword.'&limit=100&user_fields=id,first_name,last_name,professional_experience');
        //  $oTotal    = $oResponse->contacts->users;
        //  $iTotal    = $oResponse->contacts->total;
